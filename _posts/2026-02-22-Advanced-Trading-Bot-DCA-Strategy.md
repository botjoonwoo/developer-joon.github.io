...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 초기 하락      소량 추가         신중한 진입
 중기 하락      점진적 증가       평단가 관리 시작
 심화 하락      비중 확대         공격적 평단 낮춤
 극심 하락      최대 투입         최종 방어선
```

**핵심 포인트**: 
- N단계 하락 구간별로 차등 분배하여 극심한 폭락 시에도 평단가를 관리 가능한 수준으로 유지
- 후반 단계로 갈수록 더 큰 비중을 투입하는 구조
- 구체적인 수치는 자체 백테스팅 결과를 기반으로 설정
...
class DCAManager:
    def __init__(self, initial_capital: float, base_price: float):
        ...
        self.base_investment = initial_capital * INITIAL_RATIO  # 자체 최적화된 초기 투자 비율
        
        # N단계 DCA 레벨 정의
        # 구체적 수치는 백테스팅 결과 기반 설정
        # 원칙: 후반 단계로 갈수록 더 큰 비중 투입
        self.levels: List[DCALevel] = self._load_dca_config()
    ...
    def _load_dca_config(self) -> List[DCALevel]:
        """DCA 설정 로드 (환경변수 또는 별도 설정 파일)"""
        # 실제 구현에서는 자체 백테스팅 결과를 반영한 설정 사용
        # 예시: 점진적 하락률과 증가하는 투입 비율
        return [
            # DCALevel(level, trigger_drop_pct, buy_ratio)
            # ... (구체적 수치는 백테스팅 기반 최적화)
        ]
...
class HybridSignalGenerator:
    ...
    def generate_signal(self, news_list: List[Dict] = None) -> str:
        ...
        # 3. 하이브리드 점수 계산
        # 가중 평균 (세부 로직 생략 - 자체 최적화된 가중치 적용)
        hybrid_score = self._calculate_hybrid_score(random_value, sentiment)
        
        # 4. 시그널 변환 (임계값 기반)
        if hybrid_score > SIGNAL_THRESHOLD:
            return 'BUY'
        elif hybrid_score < -SIGNAL_THRESHOLD:
            return 'SELL'
        else:
            return 'HOLD'
    
    def _calculate_hybrid_score(self, random_val: float, sentiment: float) -> float:
        """하이브리드 점수 계산 (자체 최적화 가중치)"""
        # 구체적 가중치는 백테스팅 결과 기반
        # ... (구현 생략)
        pass
```
...
- **액션**: 다단계 방어 임계값 적용
  - **1단계 알림**: 텔레그램 알림만 발송 (수동 판단 여지)
  - **2단계 감축**: 포지션 일부 자동 청산
  - **3단계 청산**: 남은 포지션 전량 청산 + 봇 중지
  
> 구체적인 임계값은 백테스팅과 리스크 허용도에 따라 개인화 필요
...
class PhaseDefenseManager:
    ...
    def check_defense_trigger(self, current_price: float, position_size: float) -> dict:
        ...
        # 다단계 방어 임계값 적용
        # 임계값은 백테스팅 기반 설정 - 리스크 허용도에 따라 개인화 필요
        
        # 3단계: 전량 청산
        if drop_from_avg <= LIQUIDATE_THRESHOLD:
            self.notifier.send(
                f"🚨 <b>긴급 청산 실행</b>\n\n"
        ...
        # 2단계: 부분 감축
        if drop_from_avg <= REDUCE_THRESHOLD and not self.reduced_at_5pct:
            ...
            return {
                ...
                'reason': f'평단가 대비 {drop_from_avg:.2f}% 하락',
                'sell_ratio': REDUCE_RATIO  # 자체 설정값
            }
        
        # 1단계: 알림
        if drop_from_avg <= ALERT_THRESHOLD and not self.alert_sent_3pct:
...
암호화폐 시장은 짧은 시간에 급격히 움직이는 일이 흔합니다. 급변 시 **즉시 알림**을 받아 수동 개입 여지를 만듭니다.
...
class VolatilityMonitor:
    def __init__(self, threshold_pct: float = VOLATILITY_THRESHOLD, 
                 window_minutes: int = MONITOR_WINDOW):
        """
        threshold_pct: 알림 발동 변동률 (자체 설정값)
        window_minutes: 감시 시간 창 (분) (개념: 단기 급변 감지)
        """
        self.threshold = threshold_pct
        self.window_minutes = window_minutes
        self.price_history = deque(maxlen=window_minutes)  # 최근 N분 가격
...
class LimitOrderManager:
    ...
    def register_dca_orders(self, symbol: str, dca_manager) -> List[Dict]:
        ...
        개념:
        1. DCA 레벨별 트리거 가격 계산
        2. 각 레벨에 대응하는 지정가 매수 주문 등록
        3. 주문 ID 추적하여 체결 모니터링
        """
        orders = []
        ...
        for level in dca_manager.levels:
            ...
            # 1. 트리거 가격 및 수량 계산 (로직 생략)
            trigger_price = self._calculate_trigger_price(level, dca_manager)
            buy_quantity = self._calculate_order_quantity(level, dca_manager, trigger_price)
            ...
            try:
                ...
                orders.append({'level': level.level, 'order_id': order['id']})
                logger.info(f"DCA Level {level.level} 주문 등록 완료")
            
            except Exception as e:
                logger.error(f"주문 등록 실패: {e}")
        
        return orders
    
    def register_take_profit_orders(self, symbol: str, avg_price: float, 
                                     total_coins: float) -> List[Dict]:
        """
        익절 주문 사전 등록 (다단계 분할 익절)
        
        개념:
        1. 목표 수익률별로 익절 가격 계산
        2. 포지션을 분할하여 지정가 매도 주문 등록
        3. 단계적 이익 실현 구조
        """
        orders = []
        
        # 다단계 분할 익절 (구체적 비율은 백테스팅 기반)
        targets = self._get_take_profit_targets()
        coins_per_target = total_coins / len(targets)
        
        for profit_pct in targets:
            ...
            try:
                ...
                self.active_orders.append(order['id'])
                orders.append({'target': f'+{profit_pct}%', 'order_id': order['id']})
                logger.info(f"익절 주문 등록 (+{profit_pct}%)")
    ...
    def _get_take_profit_targets(self) -> List[float]:
        """익절 목표 수익률 (자체 최적화 값)"""
        # 구체적 수치는 백테스팅 결과 반영
        # 개념: 점진적으로 높은 수익률에 소량씩 매도
        pass
...
order_manager.register_take_profit_orders(
    ...
    total_coins=position_size
    # 내부적으로 다단계 분할 익절 목표 적용
)
```
...
class BybitHedgingManager:
    ...
    def calculate_hedge_ratio(self, sentiment_score: float) -> float:
        ...
        개념:
        - BULLISH (긍정적 감성): 낮은 헤지 비율 (상승 기대)
        - NEUTRAL (중립): 중간 헤지 비율 (리스크 중립)
        - BEARISH (부정적 감성): 높은 헤지 비율 (하락 방어)
        """
        # 구체적 임계값과 비율 계산식은 백테스팅 기반 최적화
        # ... (세부 로직 생략)
        
        if sentiment_score >= BULLISH_THRESHOLD:
            return self._calc_bullish_hedge(sentiment_score)
        elif sentiment_score <= BEARISH_THRESHOLD:
            return self._calc_bearish_hedge(sentiment_score)
        else:
            return self._calc_neutral_hedge(sentiment_score)
...
**이 글에서 소개한 전략의 세부 파라미터는 자체 백테스팅을 통해 최적화한 값입니다.** 

각자의 투자 환경과 리스크 허용도가 다르므로, 단순히 코드를 복사하기보다는:

- **자신만의 백테스팅**을 통해 파라미터 최적화
- **리스크 허용도**에 맞는 DCA 비율 및 방어 임계값 설정
- **시장 분석 능력**과 **거래 스타일**을 반영한 시그널 가중치 조정

을 권장합니다. 시장 상황과 개인의 투자 성향은 모두 다르므로, 본인에게 맞는 설정을 찾는 것이 가장 중요합니다.
